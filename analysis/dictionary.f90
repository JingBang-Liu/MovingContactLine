MODULE dictionary
  ...
  CHARACTER(len=100) :: filename_bins
  ...
  INTEGER(INT64) :: n_lines ! number of lines the data file have
  INTEGER :: time_marks ! number of iterations
  INTEGER :: n_type_1, n_type_2, n_type_3
  REAL(REAL32), DIMENSION(2) :: XD, YD, ZD  ! size of simulation box
  ...
  REAL(REAL32), DIMENSION(:,:,:,:), ALLOCATABLE :: bins_1, bins_2, bins
  INTEGER, DIMENSION(:,:), ALLOCATABLE :: contact_line_index_1
  REAL(REAL32), DIMENSION(2) :: tol_1, tol_1_all
  REAL(REAL32), DIMENSION(2) :: XB, YB, ZB ! size of bins
