MODULE write_netcdf
  ...
  SUBROUTINE write_bins(filename_bins)
    ...
    INTEGER, DIMENSION(ndims) :: var_id_bins
    ! file id
    INTEGER :: file_id
    ! for loop
    INTEGER :: i
    ...
    CALL ckeck(nf90_put_att(file_id, NF90_BLOBAL, 'time_marks', time_marks))

    ! define dimension ids
    DO i=1, ndims
      CALL check(nf90_def_dim(file_id, dim_bins(i), sizes_bins(i), dim_ids_bins(i)))
    END DO
